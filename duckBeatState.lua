...
function DuckBeatState:draw()
	...
	love.graphics.setColor(love.math.colorFromBytes(255,255,255,alpha))
	...
	for i=1,4 do
		if self.flash[i].time > 0 then
			...
			if self.flash[i].color == DuckBeatState.FLASH_RED then
				love.graphics.setColor(love.math.colorFromBytes(186, 18, 18, alpha))
			else
				love.graphics.setColor(love.math.colorFromBytes(84, 177, 33, alpha))
			end
